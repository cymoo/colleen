...
### Benchmark 实践（并发与内容体量）

如果你希望评估 Colleen 在不同并发和负载下的表现，建议按“固定变量 + 场景矩阵”方式进行：

1. 固定测试环境  
   - 固定 JDK 版本（建议 Java 21+）  
   - 固定机器规格（CPU/内存）与部署方式（裸机/容器）  
   - 关闭无关后台任务，避免噪声

2. 固定服务配置  
   - 保持 `maxConcurrentRequests`、超时、JSON 配置不变  
   - 每次仅改变一个变量（并发数、响应体大小、是否流式）

3. 设计基准场景矩阵  
   - **轻量文本**：`GET /ping`，返回短字符串（框架基础开销）  
   - **小 JSON**：`GET /json-small`，返回小对象（常规 API）  
   - **大 JSON（非流式）**：`GET /json-large`，`ctx.json(data)`（内存峰值敏感）  
   - **大 JSON（流式）**：`GET /json-large-stream`，`ctx.json(data, stream = true)`（吞吐与内存折中）  
   - **上传场景**：`POST /upload`，分别测试 1MB / 10MB / 50MB 文件（内容占用与背压）

4. 分级并发压测  
   - 并发阶梯可从 `50 -> 200 -> 500 -> 1000` 逐步提升  
   - 每档至少执行一次预热（30~60s）和一次正式采样（60~180s）  
   - 关注是否出现超时、拒绝、GC 抖动或延迟突刺

可用 `wrk` 或 `hey` 进行快速压测（示例）：

```bash
# 200 并发，8 线程，持续 60 秒（小响应）
wrk -t8 -c200 -d60s --latency http://127.0.0.1:7070/ping

# 500 并发，持续 90 秒（大 JSON 流式）
wrk -t8 -c500 -d90s --latency http://127.0.0.1:7070/json-large-stream

# 200 并发，10MB 请求体（示例，使用 hey）
hey -n 2000 -c 200 -m POST -T "application/octet-stream" -D ./10mb.bin http://127.0.0.1:7070/upload
```
